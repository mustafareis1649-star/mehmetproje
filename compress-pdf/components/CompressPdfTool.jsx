...
import { compressPdfDicts } from '../i18n';
import { compressPdf, formatSize } from '../logic/compressPdf';

const LEVELS = ['low', 'medium', 'high'];

export default function CompressPdfTool() {
  const t = useToolI18n(compressPdfDicts);
  ...
  async function handleCompress() {
    ...
    try {
      const { blob, fileName, originalSize, newSize } = await compressPdf(file, level, (current, total) => {
        setProgress(Math.round((current / total) * 100));
        setStatus(t('status_compressing').replace('{current}', current).replace('{total}', total));
      });
      const pct = Math.max(0, Math.round((1 - newSize / originalSize) * 100));
      setStatus(t('status_done').replace('{percent}', pct));
          ...
          <div className="pdf-ws-card">
            ...
            <div className="pdf-ws-footer">
              {result ? (
                ...
                <button className="btn btn-signal pdf-ws-primary" onClick={handleCompress}>{t('compress_btn')}</button>
              ) : null}
