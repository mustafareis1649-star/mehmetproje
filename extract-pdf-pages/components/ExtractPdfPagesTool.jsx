...
import { extractPdfPagesDicts } from "../i18n";
import { extractPdfPages, parsePageSelection, formatSize } from "../logic/extractPdfPages";
import { PDFDocument } from "pdf-lib";

export default function ExtractPdfPagesTool() {
  const t = useToolI18n(extractPdfPagesDicts);
  ...
  async function handleExtract() {
    if (!pagesInput.trim()) { setError(t("need_pages")); return; }
    setBusy(true); setError("");
    try {
      const pageIndices = parsePageSelection(pagesInput, pageCount);
      const { blob, fileName } = await extractPdfPages(file, pageIndices);
  ...
  function reset() {
    ...
    setFile(null); setResult(null); setError(""); setPagesInput(""); setPageCount(null);
  }
          ...
          <div className="pdf-ws-card">
              ...
              <div className="pdf-ws-left">
                ...
                {file && !result && (
                  <div className="pdf-ws-controls">
                    <label className="pdf-ws-ctrl-label">{t("pages_label")}{pageCount ? ` (1–${pageCount})` : ""}</label>
                    <input type="text" className="pdf-ws-ctrl-input" value={pagesInput}
                      onChange={(e) => setPagesInput(e.target.value)} placeholder={t("pages_placeholder")} />
                  </div>
                )}
              </div>
            ...
            <div className="pdf-ws-footer">
              {result ? (
                ...
                <button className="btn btn-signal pdf-ws-primary" onClick={handleExtract} disabled={busy}>{busy ? t("extracting") : t("extract_btn")}</button>
              ) : null}
